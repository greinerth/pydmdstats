...
from util.experiment_utils import (OPT_ARGS, comp_checker, exec_times_bop_dmd,
...
def test_complex2d_signal(method: str,
                          n_runs: int,
                          std: float,
    ...
    data = [np.expand_dims(np.array([2 / np.cosh(x1grid) / np.cosh(x2grid)
            * (1.2j**-t)]), axis=-1) for t in time]
    ...
    for i in range(snapshots_flat.shape[-1]):
        ...
        dmd_rec.append(np.concatenate(
            [__dmd_rec.real, __dmd_rec.imag], axis=-1))
        data_in.append(np.concatenate(
            [data[i].real, data[i].imag], axis=-1))
...
def test_2_moving_points(method: str,
                         n_runs: int,
                         std: float,
                         eps: float) -> Dict[str, Any]:
    ...
    for i in range(__flat.shape[-1]):
        __dmd_rec = np.expand_dims(
            __dmd_pred[:, i].reshape(imgs.shape[1:]), axis=0)
...
def test_global_temp(method: str,
                     n_runs: int,
                     std: float,
                     eps: float) -> Dict[str, Any]:
    ...
    for i in range(__flat.shape[-1]):
        ...
        __dmd_rec = np.expand_dims(
            __flat_in.reshape([1] + list(sst[0].shape)), axis=-1)
    ...
    return {"case": "Global temperature",
            "omega_size": omega_size,
            "method": method,
            "SSIM": (mean, var),
            "compression": eps,
